...
# kubemaster.com/db-operator-bundle:$VERSION and kubemaster.com/db-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= obeleh/db-operator
...
generate-deploys:
	...
	tail -n +14 db-operator-single-file-deploy.yaml >> helm/charts/db-operator/templates/db-operator-single-file-deploy.yaml
...
kuttl-test-cockroachdb:
	mkdir -p tests/outputs 
	kubectl kuttl test --config kuttl-test-cockroachdb.yaml
